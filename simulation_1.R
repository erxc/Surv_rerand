# =====================================================================
# MASTER SCRIPT: SCENARIO 1 (Proportional Hazards)
# Estimators: IPCW-KM and DML
# =====================================================================
...
for (m in 1:n_reps) {
  ...
  tryCatch({
    ...
    for (design in designs) {
      ...
      # ---------------------------------------------------
      # RUN ESTIMATOR 1: IPCW-KM
      # ---------------------------------------------------
      res_ipcw <- run_IPCW(
        time = X_obs, event = Delta_obs, treat = A_obs, 
        W_cens = W_all, Z_rr = Z_rr
      )
      ...
      for (k in 1:length(t_eval)) {
        ...
        results_list[[counter]] <- data.table(
          ...
          Estimator = "IPCW",
          Time = t_eval[k],
          Estimate = res_ipcw$Estimate[k],
          Var_Estimated = res_ipcw$Var_Deflated[k],
          CI_Lower = res_ipcw$CI_Lower[k],
          CI_Upper = res_ipcw$CI_Upper[k],
          Uniform_CB_Lower = res_ipcw$Uniform_CB_Lower[which(t_grid == t_eval[k])],
          Uniform_CB_Upper = res_ipcw$Uniform_CB_Upper[which(t_grid == t_eval[k])]
        )
        counter <- counter + 1
...
file_name <- paste0("Simulation_Results_Scenario1_20260422_n", n, ".csv")
